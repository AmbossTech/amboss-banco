...
const u = {
  ...
  hexEncode: hexToBytes,
  utf8Encode: utf8ToBytes,
  utf8Decode(bytes: Uint8Array) {
    return decoder.decode(bytes);
  },

  ensureBytes(data: Uint8Array, size: number) {
    if (data.length !== size) {
      throw new Error(
        `Invalid byte length of ${data.length} instead of ${size}`
      );
    }
  },
  ...
  getMessageKeys(conversationKey: Uint8Array, nonce: Uint8Array) {
    u.ensureBytes(conversationKey, 32);
    u.ensureBytes(nonce, 32);
